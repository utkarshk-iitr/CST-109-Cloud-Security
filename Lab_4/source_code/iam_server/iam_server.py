    ...
    def _login(self, request, client_ip):
        ...
        with self.lock:
            ...
            if now < lock_until:
                ...
                auth_log.warning(
                    "LOGIN BLOCKED | user=%s ip=%s reason=locked remaining=%ss",
                    username,
                    client_ip,
                    remaining,
                )
            ...
            if current != expected:
                ...
                auth_log.warning(
                    "LOGIN FAILED | user=%s ip=%s reason=bad_password attempt=%s",
                    username,
                    client_ip,
                    attempts,
                )
                if attempts >= MAX_LOGIN_ATTEMPTS:
                    ...
                    mitigation_log.critical(
                        "ACCOUNT LOCKOUT | user=%s ip=%s attempts=%s lock_seconds=%s",
                        username,
                        client_ip,
                        attempts,
                        ACCOUNT_LOCK_SECONDS,
                    ...
                    return {
                        "status": "ERROR",
                        "message": f"Account locked for {ACCOUNT_LOCK_SECONDS} seconds",
                    }
                return {"status": "ERROR", "message": "Invalid credentials"}
