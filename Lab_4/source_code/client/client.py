...
class IAMClient:
    ...
    def brute_force_simulation(self):
        ...
        print("\n[Attack] Brute-force login simulation\n")
    ...
    def invalid_token_attack(self):
        print("\n[Attack] Invalid/tampered token simulation\n")
        fake_tokens = [
            "invalid.token.value",
            "abc.def.ghi",
        ...
        for idx, token in enumerate(fake_tokens, start=1):
            ...
            print(
                f"Test {idx}: token='{token[:20]}' -> {response.get('status')} | {response.get('message')}"
            )
            time.sleep(0.1)
