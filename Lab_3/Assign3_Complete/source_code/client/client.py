...
CA_CERT  = os.path.join(_SRC_DIR, 'certs', 'server.crt')
...
class CloudClient:
    ...
    def send_request(self, request):
        try:
            ...
            raw.settimeout(10)
            sock = self._ssl_ctx.wrap_socket(raw, server_hostname='localhost')
    ...
    def upload_file(self, filepath):
        ...
        request = {'operation': 'UPLOAD_REQUEST','token': self.token,'filename': filename,'file_size': file_size,'num_chunks': n}
        response = self.send_request(request)
        ...
        with open(filepath, 'rb') as f:
            for allocation in response['chunk_allocations']:
                ...
                try:
                    ...
                    store_req = {'operation': 'STORE_CHUNK','filename': filename,'chunk_id': chunk_id,'chunk_size': chunk_size}
        ...
        complete_req = {'operation': 'UPLOAD_COMPLETE','token': self.token,'filename': filename,'chunks': chunks_info,'total_size': file_size}
    ...
    def download_file(self, filename, output_path=None):
        ...
        request = {'operation': 'DOWNLOAD_REQUEST','token': self.token,'filename': filename}
        response = self.send_request(request)
        ...
        with open(output_path, 'wb') as f:
            for chunk_info in sorted(chunks, key=lambda x: x['chunk_id']):
                ...
                try:
                    ...
                    req = {'operation': 'RETRIEVE_CHUNK','filename': filename,'chunk_id': chunk_id}
    ...
    def delete_file(self, filename):
        ...
        request = {'operation': 'DELETE_FILE','token': self.token,'filename': filename}
        response = self.send_request(request)
    ...
    def brute_force(self, target_user='admin', attempts=10):
        print(f"\n  {'='*55}")
        print(f"  ATTACK SIMULATION: Brute-Force Login")
        print(f"  Target: {target_user} | Attempts: {attempts}")
        print(f"  {'='*55}\n")

        passwords = ['wrong1', 'pass123', 'letmein', 'password', '123456','qwerty', 'abc123', 'monkey', 'dragon', 'master','test', 'trustno1', 'baseball', 'shadow', 'hello']
        start = time.time()
        blocked_at = None

        for i in range(attempts):
            pwd = passwords[i % len(passwords)]
            request = {'operation': 'LOGIN','username': target_user,'password': pwd}
            response = self.send_request(request)
    ...
    def dos(self, num_requests=50, threads=5):
        print(f"\n  {'='*55}")
    ...
    def token_tampering(self):
        ...
        mitg1 = response['status'] == 'ERROR'

        print("\n  [Test 2] Tampering with a valid token structure...")
        import base64
        tampered = base64.urlsafe_b64encode(b'{"user":"admin","role":"admin"}').decode()
        response = self.send_request({'operation': 'DELETE_FILE','token': tampered,'filename': 'secret.txt'})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        mitg2 = response['status'] == 'ERROR'

        print("\n  [Test 3] Using empty/null token...")
        response = self.send_request({'operation': 'UPLOAD_REQUEST','token': '','filename': 'test.txt','file_size': 100,'num_chunks': 1})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        mitg3 = response['status'] == 'ERROR'

        print("\n  [Test 4] Using token with modified payload...")
        response = self.send_request({'operation': 'DELETE_FILE','token': 'eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiYWRtaW4iLCJyb2xlIjoiYWRtaW4ifQ.tampered','filename': 'important.txt'})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        mitg4 = response['status'] == 'ERROR'

        total_mitigated = sum([mitg1, mitg2, mitg3, mitg4])
        print(f"\n  SUMMARY:")
        print(f"    Tests run:     4")
    ...
    def replay_attack(self):
        ...
        print("  [Step 1] Obtaining a old legitimate token...")
        response = self.send_request({'operation': 'LOGIN','username': 'user1','password': 'user123','token_ttl':1})
        if response['status'] != 'SUCCESS':
            print(f"    Failed to get token: {response.get('message')}")
            return
        old_token = response['token']
        print(f"    Old Token captured: TTL = 1 sec ...")
        time.sleep(2)
        print("\n  [Step 2] Using old token (invalid use)...")
        response = self.send_request({
            'operation': 'LIST_FILES',
            'token': old_token
        })
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        mitgreplay = response['status'] == 'ERROR'

        print(f"    Replay defenses active:")
        print(f"      - Old token rejected: {'YES' if mitgreplay else 'NO'}")
    ...
    def idor(self):
        ...
        response = self.send_request({'operation': 'LOGIN','username': 'user1','password': 'user123'})
        if response['status'] != 'SUCCESS':
        ...
        response = self.send_request({'operation': 'DELETE_FILE','token': user_token,'filename': 'admin_secret.txt'})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        delete_blocked = response.get('message') == 'Permission denied'

        print("\n  [Step 3] Attempting to download non-owned file by name manipulation...")
        response = self.send_request({'operation': 'DOWNLOAD_REQUEST','token': user_token,'filename': '../../../etc/passwd'})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
        traversal_blocked = response['status'] == 'ERROR'

        print("\n  [Step 4] Login as readonly user, attempt upload (privilege test)...")
        response = self.send_request({'operation': 'LOGIN','username': 'readonly','password': 'read123'})
        ro_token = response.get('token')
        if ro_token:
            response = self.send_request({'operation': 'UPLOAD_REQUEST','token': ro_token,'filename': 'malicious.txt','file_size': 100,'num_chunks': 1})
            print(f"    Result: {response['status']} - {response.get('message', '')}")
        ...
        response = self.send_request({'operation': 'LIST_FILES'})
        print(f"    Result: {response['status']} - {response.get('message', '')}")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            client.brute_force(user, attempts)
            ...
            client.dos(num, threads)

        elif choice == '8':
            client.token_tampering()

        elif choice == '9':
            client.replay_attack()
