...
# TLS — shared self-signed certificate (generated by gen_certs.py)
...
class CloudClient:
    ...
    def _make_ssl_ctx(self):
        """Build a reusable TLS client context that trusts our self-signed cert."""
        ctx = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
        ctx.load_verify_locations(CA_CERT)
        ctx.check_hostname = False  # IP-based connections (127.0.0.1)
        return ctx

    def send_request(self, request):
        """Send JSON request to API Gateway over TLS and return response dict."""
        try:
    ...
    def connect_storage(self, host, port):
        """Direct TLS connection to a storage server for data transfer."""
        raw = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        raw.settimeout(30)
        sock = self._ssl_ctx.wrap_socket(raw, server_hostname='localhost')
