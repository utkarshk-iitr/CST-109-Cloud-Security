...
def init_log(sid):
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'../../logs/{sid}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class SecMonitor:
    def __init__(self, sid):
        ...
        handler = logging.FileHandler(f'../../logs/threats_{sid}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
class IntegrityManager:
    def __init__(self, sid):
        self.sid = sid
        self.cksums = {}
        self.cksum_file = f'../../logs/{sid}_checksums.json'
        self.lock = threading.Lock()
        self.load()
...
class StorageServer:
    def __init__(self, host, port, sid):
        ...
        ah = logging.FileHandler(f'../../logs/audit_{sid}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
