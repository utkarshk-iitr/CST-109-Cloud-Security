...
APPLICATION_SERVER_HOST = '10.81.1.104'
...
logging.basicConfig(
    ...
    format=LOG_FORMAT,
    handlers=[
        logging.FileHandler(f'../../logs/application_server_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
security_handler = logging.FileHandler(f'../../logs/auth_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
security_handler.setFormatter(logging.Formatter(LOG_FORMAT))
security_logger.addHandler(security_handler)
security_logger.setLevel(logging.INFO)

threat_logger = logging.getLogger('ThreatDetection')
threat_handler = logging.FileHandler(f'../../logs/threats_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
