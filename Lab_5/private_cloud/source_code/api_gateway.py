...
class APIGateway:
    def __init__(self):
        ...
        self.back_ids = {backend["id"] for backend in BACKEND_SERVERS}
        init_key_ring()

        self.perm = {"GET_PROFILE": {"admin", "user"}, "GET_ADMIN_REPORT": {"admin"},"SHOW_ENC_REC": {"admin"},}
    ...
    def blocked(self, client_ip):
        ...
        with self.lock:
            ...
            if until > now: return True, int(until - now)
            if until: del self.blocked_ips[client_ip]
            return False, 0

    def record_inv(self, client_ip):
        with self.lock:
    ...
    def record_val(self, client_ip):
        with self.lock: self.inv_cnt[client_ip] = 0

    def track_viol(self, username, reason, client_ip):
        with self.lock:
            self.user_inv_cnt[username] += 1
            count = self.user_inv_cnt[username]
            if count >= UNAUTH_THRESHOLD:
                self.blocked_until[username] = time.time() + LOCK_SEC
                self.node_restrict[username] = {"deny_nodes": {"backend_2"},"until": time.time() + LOCK_SEC,}
                mitg_log.critical("USER BLOCKED | user=%s ip=%s reason=%s threshold=%s LOCK_SEConds=%s",username,client_ip,reason,UNAUTH_THRESHOLD,LOCK_SEC,)
                return True
            return False
    ...
    def is_block(self, username):
        now = time.time()
        with self.lock:
            until = self.blocked_until.get(username, 0)
            if until > now: return True, int(until - now)
            if until: self.blocked_until.pop(username, None)
            return False, 0

    def record_pattern(self, username, operation, client_ip):
        now = time.time()
        with self.lock:
            window = [ts for ts in self.user_access[username] if now - ts < 60]
            window.append(now)
            self.user_access[username] = window
            if len(window) >= UNUSUAL_ACCESS:
                threat_log.critical("ALERT | user=%s ip=%s op=%s reason=unusual_access_frequency hits_per_min=%s",username,client_ip,operation,len(window),)
                self.node_restrict[username] = {"deny_nodes": {"backend_2"},"until": time.time() + LOCK_SEC,}
    ...
    def cleanup(self):
        while True:
            ...
            with self.lock:
                ...
                expired_users = [usr for usr, until in self.blocked_until.items() if until <= now]
                for usr in expired_users:
                    self.blocked_until.pop(usr, None)
                    self.user_inv_cnt[usr] = 0

                expired_restrict = [usr for usr, rule in self.node_restrict.items() if rule.get("until", 0) <= now]
                for usr in expired_restrict:
                    self.node_restrict.pop(usr, None)
            time.sleep(5)
    ...
    def get_rest(self, username):
        with self.lock:
            rule = self.node_restrict.get(username)
            if rule is None:
                return set()
            if rule.get("until", 0) <= time.time():
                self.node_restrict.pop(username, None)
                return set()
            return set(rule.get("deny_nodes", set()))

    def bck(self, username):
        denied = self.get_rest(username)
    ...
    def auth(self, request, client_ip):
        ...
        if not valid:
            blocked = self.record_inv(client_ip)
        ...
        self.record_val(client_ip)

        user = payload.get("sub", "unknown")
        user_blocked, remaining = self.is_block(user)
        if user_blocked:
        ...
        if role not in self.perm.get(operation, set()):
            ...
            blocked = self.track_viol(user, "rbac", client_ip)
            if blocked:
        ...
        self.record_pattern(user, operation, client_ip)
    ...
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            backend = self.bck(claims.get("sub"))
            if backend is None:
            ...
            bck_id = back_resp.get("server", backend["id"])
            if bck_id not in self.back_ids:
                threat_log.critical("ALERT | user=%s ip=%s op=%s reason=unknown_node backend=%s",claims.get("sub"),client_ip,operation,bck_id,)
    ...
    def start(self):
        ...
        clean_th = threading.Thread(target=self.cleanup, daemon=True)
        clean_th.start()
        key_thread = threading.Thread(target=self.rotate_keys, daemon=True)
        key_thread.start()
        ...
        try:
            while True:
                ...
                if tls is not None:
                    try: conn = tls.wrap_socket(conn, server_side=True)
