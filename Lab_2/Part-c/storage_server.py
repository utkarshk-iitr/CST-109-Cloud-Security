...
class StorageServer:
    ...
    def handle_client(self, client_socket, address):
        try:
            ...
            if op == 'STORE_CHUNK':
                self.store_count += 1
                response = self.handle_store(client_socket, request)
            elif op == 'RETRIEVE_CHUNK':
                self.retrieve_count += 1
                response = self.handle_retrieve(client_socket, request)
            ...
            try: client_socket.send(json.dumps(error_response).encode())
            except:
                pass
        finally:
            client_socket.close()
    ...
    def handle_health_check(self):
        return {'status': 'SUCCESS','server_id': self.server_id,'online': True}
    ...
    def handle_store(self, client_socket, request):
        ...
        replica_type = "REPLICA" if is_replica else "PRIMARY"
        ...
        with open(chunk_path, 'wb') as f:
            f.write(chunk_data)
        self.logger.info(f"Stored {replica_type} chunk {chunk_id} of {filename} ({len(chunk_data)} bytes)")
        response = {'status': 'SUCCESS','message': f'Chunk stored successfully as {replica_type}','chunk_id': chunk_id}
        client_socket.send(json.dumps(response).encode())
        return response
    ...
    def start(self):
        ...
        self.logger.info(f"Replication enabled (factor: 3)")
        self.logger.info(f"Fault tolerance enabled")
        ...
        try:
            ...
            self.logger.info(f"  Store operations: {self.store_count}")
            self.logger.info(f"  Retrieve operations: {self.retrieve_count}")
            self.logger.info(f"  Health checks: {self.health_check_count}")
        finally:
            server_socket.close()

if len(sys.argv) != 2:
    print("Usage: python storage_server.py <server_number>")
    sys.exit(1)
