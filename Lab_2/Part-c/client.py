...
class DFSClient:
    ...
    def download_file(self, filename, output_path=None):
        ...
        with open(output_path, 'wb') as f:
            for chunk_info in sorted(chunks, key=lambda x: x['chunk_id']):
                ...
                for attempt, replica in enumerate(replicas, 1):
                    ...
                    try:
                        ...
                        if response['status'] != 'SUCCESS':
                            print(f"Attempt {attempt}: {server_id}: {response['message']}")
                            storage_sock.close()
                            continue
                        
                        received_chunk_size = response['chunk_size']
                        storage_sock.send(b'READY')
                        
                        chunk_data = b''
                        remaining = received_chunk_size
                        while remaining > 0:
                        ...
                        print(f"Attempt {attempt}: {server_id}: {e}")
                        continue
    ...
    def show_server_status(self):
        try:
            sock = self.connect_metadata()
            request = {'operation': 'SERVER_STATUS'}
            sock.send(json.dumps(request).encode())
            response = json.loads(sock.recv(4096).decode())
            sock.close()
            ...
            print(f"\n{'='*70}")
            print(f"Storage Server Status")
            print(f"{'='*70}")
            print(f"{'Server ID':<15} {'Host:Port':<25} {'Status':<10}")
            print(f"{'-'*70}")
            
            for server in servers:
                status_icon = "🟢 ONLINE" if server['status'] == 'online' else "🔴 OFFLINE"
                host_port = f"{server['host']}:{server['port']}"
                print(f"{server['server_id']:<15} {host_port:<25} {status_icon:<10}")
            
            online_count = sum(1 for s in servers if s['status'] == 'online')
            print(f"{'-'*70}")
            print(f"Summary: {online_count}/{len(servers)} servers online")
            print(f"{'='*70}\n")
...
def print_menu():
    print("\n" + "="*70)
    print("Distributed File System Client (With Fault Tolerance)\n")
    print("1. Upload file")
    print("2. Download file")
    print("3. List files")
    print("4. Show server status")
    print("5. Exit")
    print("="*70)
